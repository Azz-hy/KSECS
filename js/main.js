  ...
  function buildVideoCard(video, targetId) {
    ...
    if (isDrive) {
      ...
      fsOverlay.addEventListener("click", (e) => {
        ...
        // Try the Native HTML5 Fullscreen API first.
        if (nativeFs) {
          try {
        ...
        // Some Android / in-app browsers (Instagram, Facebook, TikTok webviews,
        // some Samsung Internet versions) silently no-op requestFullscreen:
        // no error, no rejected promise, nothing visibly happens. So instead of
        // trusting the promise/exception alone, verify the real state shortly
        // after and fall back to the CSS fullscreen if it never engaged.
        setTimeout(() => {
      ...
      fsClose.addEventListener("click", (e) => {
        ...
        const isCurrentlyFullscreen =
          document.fullscreenElement || document.webkitFullscreenElement ||
          document.mozFullScreenElement || document.msFullscreenElement;
        if (exitFs && isCurrentlyFullscreen) {
          try { exitFs.call(document); } catch (err) {}
        }
      });
